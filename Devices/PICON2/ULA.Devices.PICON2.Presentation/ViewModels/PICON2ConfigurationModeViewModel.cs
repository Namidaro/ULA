    ...
    public class PICON2ConfigurationModeViewModel : ViewModelBase, IConfigurationModeViewModel, INavigationAware
    {
        ...
        private async void OnSendConfiguration()
        {
            ...
            try
            {
                ...
                await (this._currentDeviceViewModel.Model as IRuntimeDevice).TcpDeviceConnection.PostDataByAddressAsync(ADDRESS, resultPackage.Take(LENGTH_WORD / 2).ToArray(),
                    RequestStrings.SEND_CONFIGURATION);

                await (this._currentDeviceViewModel.Model as IRuntimeDevice).TcpDeviceConnection.PostDataByAddressAsync(ADDRESS + LENGTH_WORD / 4, resultPackage.Skip(LENGTH_WORD / 2).Take(LENGTH_WORD / 2).ToArray(),
                    RequestStrings.SEND_CONFIGURATION);

                await (this._currentDeviceViewModel.Model as IRuntimeDevice).TcpDeviceConnection.PostDataByAddressAsync(ADDRESS + LENGTH_WORD / 2, resultPackage.Skip(LENGTH_WORD).Take(LENGTH_WORD / 2).ToArray(),
                    RequestStrings.SEND_CONFIGURATION);

                await (this._currentDeviceViewModel.Model as IRuntimeDevice).TcpDeviceConnection.PostDataByAddressAsync(ADDRESS + LENGTH_WORD * 3 / 4, resultPackage.Skip(LENGTH_WORD * 3 / 2).Take(LENGTH_WORD / 2).ToArray(),
                    RequestStrings.SEND_CONFIGURATION);
            ...
            try
            {
                await (_currentDeviceViewModel.Model as IRuntimeDevice).InitializeAsync();
            }
