        ...
        where TKey : IComparable<TKey>
    {
        ...
        private sealed class MinBinaryHeap
        {
            ...
            public void Insert(KeyValuePair<TKey, TValue> entry)
            {
                ...
                if (pos == 0 || _items[pos].Key.CompareTo(_items[pos - 1].Key) >= 0) return;
                var nextPos = pos / 2;
                while (pos > 0)
                {
                    ...
                    var toCheckDown = default(KeyValuePair<TKey, TValue>);

                    if (nextPos - 1 >= 0)
                    {
                        toCheckDown = _items[nextPos - 1];
                    }

                    if (entry.Key.CompareTo(toCheck.Key) >= 0)
                    {
                        if (entry.Key.CompareTo(toCheck.Key) < 0 && entry.Key.CompareTo(toCheckDown.Key) >= 0)
                        {
                            _items.Insert(nextPos, entry);
                            break;
                        }
