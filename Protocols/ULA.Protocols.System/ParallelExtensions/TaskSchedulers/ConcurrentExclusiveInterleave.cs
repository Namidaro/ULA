...
namespace YP.Toolkit.System.ParallelExtensions.TaskSchedulers
{
    /// <summary>
    ...
    public sealed class ConcurrentExclusiveInterleave
    {
        ...
        public ConcurrentExclusiveInterleave(TaskScheduler targetScheduler, bool exclusiveProcessingIncludesChildren)
        {
            ...
            this._parallelOptions = new ParallelOptions { TaskScheduler = targetScheduler };
        ...
        internal void NotifyOfNewWork()
        {
            ...
            if (this._taskExecuting != null) return;
            ...
            this._taskExecuting = new Task(this.ConcurrentExclusiveInterleaveProcessor, CancellationToken.None, TaskCreationOptions.None);
            ...
            while (runTasks)
            {
                try
                {
                    ...
                    foreach (var task in GetExclusiveTasks())
                    {
                        ...
                        if (!this._exclusiveProcessingIncludesChildren || task.IsCompleted) continue;
                        cleanupOnExit = false;
                        task.ContinueWith(_ => this.ConcurrentExclusiveInterleaveProcessor(), this._parallelOptions.TaskScheduler);
                        return;
                    }
                    ...
                    Parallel.ForEach(GetConcurrentTasksUntilExclusiveExists(), this._parallelOptions,
                ...
                finally
                {
                    if (cleanupOnExit)
                    {
                        lock (this._internalLock)
                        {
                            ...
                            if (this._concurrentTaskScheduler.Tasks.Count == 0 && 
                                this._exclusiveTaskScheduler.Tasks.Count == 0)
                            {
            ...
            while (true)
            {
                ...
                lock (_internalLock)
                {
                    if (this._exclusiveTaskScheduler.Tasks.Count == 0 &&
                        this._concurrentTaskScheduler.Tasks.Count > 0)
                    {
                        foundTask = this._concurrentTaskScheduler.Tasks.Dequeue();
                    }
                }
        ...
        private IEnumerable<Task> GetExclusiveTasks()
        {
            while (true)
            {
                ...
                {
                    if (this._exclusiveTaskScheduler.Tasks.Count > 0) 
                        foundTask = this._exclusiveTaskScheduler.Tasks.Dequeue();
                }
                if (foundTask != null) yield return foundTask;
                else yield break;
            }
        ...
        private class ConcurrentExclusiveTaskScheduler : TaskScheduler
        {
            ...
            {
                var processingTaskOnCurrentThread = this._processingTaskOnCurrentThread.Value;
                if (!processingTaskOnCurrentThread) this._processingTaskOnCurrentThread.Value = true;
                base.TryExecuteTask(task);
                if (!processingTaskOnCurrentThread) this._processingTaskOnCurrentThread.Value = false;
            }
            ...
            protected override bool TryExecuteTaskInline(Task task, bool taskWasPreviouslyQueued)
            {
                if (this._processingTaskOnCurrentThread.Value)
                {
                    ...
                    t.RunSynchronously(this._interleave._parallelOptions.TaskScheduler);
                    return t.Result;
                }
                return false;
            }
